...
<body ng-controller="MainController" ng-cloak>
	...
	<div id="divContainer">
		...
		if($_COOKIE['userType']==4 || $_COOKIE['userID']==1  || $_COOKIE['userType']==5){
			...
			from stores order by stores.bakery
			";
		}
		else{
			$query ="
	  ...
	  and t1.IPRINTSTATION = 15225
	  group by t1.IPRINTSTATION, day(t1.CLOSEDATETIME), month(t1.CLOSEDATETIME)
	  order by t1.IPRINTSTATION, month desc, day desc;
		";
		$msqresult = mssql_query($msquery);
		while ($msrow = mssql_fetch_array($msqresult))
		{

			echo number_format(htmlspecialchars($msrow['cash']),2);
			echo htmlspecialchars($msrow['checks']);
			echo number_format($msrow['cash']/$msrow['checks']);
			echo = (int) $msrow['day'];
			// echo = (int) $msrow['dw'];
		}
