...
<body ng-controller="MainController">
    <nav class="navbar navbar-default">
        <a class="navbar-brand" href="/"><img alt="Brand" height="30" width="34" src="../images/fvlogo.jpg"></a>
		<a class="navbar-brand" href="/">Пекарня Ф. Вольчека</a>
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav pull-right">
				<?php
					if($_COOKIE['userType']==1) echo '<li><a href="../admin">admin</a></li>';
				?>
	...
	<div id="divContainer">	
		<?php
			...
			for($j=0; $j<$i; $j++)
			{
				echo '<div style="
							...
							" id ="'.$mass[$i][0].'">';
				echo '<div style="clear:both; text-align:center; font-family: Underdog;">Булочная № '.$mass[$j][0].' '.$mass[$j][1].'</div>';			
				$msquery="
					select * from (select top(8) convert(date,GangDateStart) as date, day(GangDateStart) as day, CashNumber as number, Summa as cash, ChequeCountSale as checks
					from OperGang				
				...
				$tempTempTemp=	"select sum(c.Summa) as total, count(c.Summa) as checks, convert(date,c.DateOperation) as date,DATEPART(hh,c.DateOperation)as time
			from ChequeHead as c
			where convert(date,c.DateOperation) > CONVERT(date,getdate()-8)
			and c.Cash_Code = 9
			group by convert(date,c.DateOperation)";
			
			
					
				$msqresult = mssql_query($msquery);
				echo "<div id='divAllTables' style='height:315px;'>";
				echo '
				...
				?>
					...
					<div ng-show="checked[<?php echo $mass[$j][0];?>]" style="
							width:99%;
							clear:both;
							border-style:solid; 
							border-width:1px; 
			...
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if($_COOKIE['userType']==1 || $_COOKIE['userID']==4)
			{
				require '../../dbconnectms.php';
				echo '<div style="
							width:229px;
				...
				$msquery="
					select convert(date,GangDateStart) as date, day(convert(date,GangDateStart)) as day, SUM(Summa) as cash, SUM(ChequeCountSale) as checks
					from OperGang				
					where convert(date,GangDateStart) > CONVERT(date,getdate()-8)	
					and Summa <> 0
				...
				echo '</table>';			
				$msquery="
						select AVG(OperGang.Summa) as avCashMonth
						, SUM(Summa) as sumCashThisMonth,AVG(OperGang.ChequeCountSale) as avChecksThisMonth,
						SUM(Summa)/SUM(OperGang.ChequeCountSale) as avCheckAmountMonth
						from OperGang 
				...
				$msquery="
					select AVG(OperGang.Summa) as avCashLastMonth
						, SUM(Summa) as sumCashLastMonth,AVG(OperGang.ChequeCountSale) as avChecksLastMonth,
						SUM(Summa)/SUM(OperGang.ChequeCountSale) as avCheckAmountLastMonth
						from OperGang 
						where YEAR(GangDateStart) = YEAR(getdate())
				...
				$msquery="
					select AVG(OperGang.Summa) as avCash, 
						AVG(OperGang.ChequeCountSale) as avChecks, SUM(Summa) as sumCash,
						SUM(Summa)/SUM(OperGang.ChequeCountSale) as avCheckAmountYear
						from OperGang 
		...
		?>
		<p class="paraOverTable" style="margin-left:10px;clear:both; padding-top:50px;">Cпасибо за то, что вы с нами! <br> Команда Ф. Вольчека.</p>
	</div>
</body>
</html>