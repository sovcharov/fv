...
<body ng-controller="MainController" ng-cloak>
	...
	<div id="divContainer">
		...
		// 		$day = "hey";//$mass[$j][5][$z]['date'];
		// 		$dw = "Hoy";//(int) $mass[$j][5][$z]['dw'];
		// 		if($day==(int) date('d'))
		// 		{
		...
		// 	echo '<tr><td style="width:30%;">Этот месяц:</td><td style="text-align:right;">'. number_format($storesData[$j]["bakeryData"]["thisMonth"]["revenue"]) . '  </td><td style="text-align:right;">'. number_format($storesData[$j]["bakeryData"]["thisMonth"]["checks"])  . '  </td><td style="text-align:right;">  ' .number_format($storesData[$j]["bakeryData"]["thisMonth"]["average"]).'</td></tr>';
		// 	echo '<tr><td style="width:30%;">Прошлый:</td><td style="text-align:right;">'. number_format($storesData[$j]["bakeryData"]["lastMonth"]["revenue"]) . '  </td><td style="text-align:right;">'. number_format($storesData[$j]["bakeryData"]["lastMonth"]["checks"])  . '  </td><td style="text-align:right;">  ' .number_format($storesData[$j]["bakeryData"]["lastMonth"]["average"]). '</td></tr>';
		// 	echo '<tr><td style="width:30%;">Позапрошлый:</td><td style="text-align:right;">'. number_format($storesData[$j]["bakeryData"]["monthBeforeLastMonth"]["revenue"]) . '  </td><td style="text-align:right;">'. number_format($storesData[$j]["bakeryData"]["monthBeforeLastMonth"]["checks"])  . '  </td><td style="text-align:right;">  ' .number_format($storesData[$j]["bakeryData"]["monthBeforeLastMonth"]["average"]). '</td></tr>';
		// 	echo '</table>';
		// 	// echo '<table>';
...
if($_COOKIE['userID']==1){
	...
	$msquery="SELECT SIFR, CODE, NAME, NETNAME FROM CASHES order by sifr desc";
	$msqresult = mssql_query($msquery);
	while ($msrow = mssql_fetch_array($msqresult))
	{
